			...
			<el-col :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['community_users_id'] && $check_field('set','gender')) || (!form_sub['community_users_id'] && $check_field('add','gender'))">
			...
			<el-col :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form_sub['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form_sub['community_users_id'] && $check_field('set','full_name')) || (!form_sub['community_users_id'] && $check_field('add','full_name'))"></el-input>
					<div v-else-if="$check_field('get','full_name')">{{form_sub['full_name']}}</div>
				</el-form-item>
			</el-col>
			<el-col :xs="24" :sm="12" :lg="8">
				<el-form-item label="住址" prop="address">
					<el-input id="address" v-model="form_sub['address']" placeholder="请输入住址"
							  v-if="user_group === '管理员' || (form_sub['community_users_id'] && $check_field('set','address')) || (!form_sub['community_users_id'] && $check_field('add','address'))"></el-input>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["community_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?community_users_id=" + form_sub["community_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/community_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/community_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/community_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/community_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/community_users/view','get');
